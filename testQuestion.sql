...
SELECT
       ...
       HIRE_DATE
  FROM EMPLOYEE
  WHERE EMP_NAME LIKE '이%'; -- 와일드 카드 사용 이로 시작하는 것을 찾겠다. 이%
...
SELECT
       ...
       DEPT_CODE
  FROM EMPLOYEE
 WHERE EMP_NAME LIKE '%하%';   -- IN (포함이라기보다는 존재하는 것만) 은 안되나?
...
SELECT
       ...
       JOB_CODE
  FROM EMPLOYEE
 WHERE JOB_CODE = 'J7' OR JOB_CODE = "J2" AND
	   SALARY >= 200;